...
export default function PodaDetail({ poda, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 backdrop-blur-sm" onClick={onClose}>
      <div
        className="bg-gradient-to-br from-white to-emerald-50/20 w-full sm:max-w-xl sm:rounded-3xl rounded-t-3xl max-h-[92dvh] overflow-y-auto overscroll-contain shadow-2xl flex flex-col"
      ...
      >
        ...
        <div className="relative px-6 pt-6 pb-4 shrink-0" style={{ background: `linear-gradient(135deg, ${color}22, ${color}08)` }}>
          ...
          <div className="flex items-center gap-2 mt-1">
            <span className="inline-block text-xs font-medium px-3 py-1 rounded-full" style={{ backgroundColor: color + "22", color }}>
              {poda.category}
            </span>
            <div className="flex items-center gap-1 bg-white/80 rounded-full px-2.5 py-0.5 border border-stone-200/60">
              <Scissors className="w-3 h-3" style={{ color }} />
              <span className="text-[11px] font-medium text-stone-600">Dificuldade: {poda.difficulty}</span>
            </div>
          </div>
